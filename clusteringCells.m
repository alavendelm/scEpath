function [y, S] = clusteringCells(data,networkIfo,C,clusterRange,showFigure,folder)
...
if isempty(C)
    for c = clusterRange
        ...
        if isequal(K1,c)
            K1 = EstimateNumberClusters(abs(S),clusterRange,1,folder);
            break;
        end
    end
else
...
function [K1, K12,eigengap] = EstimateNumberClusters(W,NUMC,showFigure,folder)
